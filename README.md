...
| WhatsApp | `NEXT_PUBLIC_AUTH_WHATSAPP=1` | Code à usage unique envoyé par WhatsApp (Infobip), ou par SMS à défaut (eSMS Africa) |
...
4. **WhatsApp / SMS** : Supabase génère le code et le confie à `/api/auth/envoi-code` (« Send SMS Hook »), qui l'envoie par WhatsApp via Infobip, puis par SMS via eSMS Africa si WhatsApp échoue ou n'est pas configuré.
   - Vercel : `SEND_SMS_HOOK_SECRET` (fourni par Supabase, `v1,whsec_…`), `INFOBIP_BASE_URL`, `INFOBIP_API_KEY`, `INFOBIP_WHATSAPP_SENDER`, `INFOBIP_TEMPLATE_NAME` (modèle Meta de catégorie « Authentification », en français ; `INFOBIP_TEMPLATE_LANGUAGE` si autre que `fr`), `ESMS_API_KEY`, `ESMS_SENDER_ID` (facultatif). Un canal sans ses variables est simplement sauté.
   - Supabase : activer le fournisseur Phone et le hook « Send SMS » vers `https://<site>/api/auth/envoi-code`.
   - Le produit « Verify » d'eSMS Africa n'est pas utilisé : il génère son propre code, que Supabase ne pourrait pas vérifier.
